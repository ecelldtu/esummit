...
export default function Events() {
    ...
    return (
        <>
            <h1 className='text-center text-5xl md:text-8xl xl:text-9xl w-full text-white py-4 font-CyberGraph'>EVENTS</h1>
            <motion.div
                ref={ref}
                animate={controls}
                initial='hidden'
                ...
                {Dep.map((person, index) => (
                    <div key={index} className='z-[500] max-w-sm rounded hover:scale-100 transition-all scale-90 overflow-hidden shadow-lg mx-auto'>
                        <div className='relative m-4 flex items-center align-middle justify-center px-auto'>
                            <Image alt='' src={person.image} width={400} height={400} className='mx-auto border-4 hover:border-violet-500 transiton-all border-white transition-all rounded-2xl bg-black object-cover' />
