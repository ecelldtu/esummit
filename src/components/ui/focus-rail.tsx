      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t border-violet-500/30 bg-violet-950/20 shadow-2xl transition-shadow duration-300",
                  isCenter ? "z-20 shadow-violet-500/20" : "z-10"
                )}
                initial={false}
                animate={{
                  x: xOffset,
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-24 justify-center">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="space-y-1"
              >
                <h2 className="text-2xl font-bold tracking-tight md:text-3xl text-white font-Kalamayka uppercase">
                  {activeItem.title}
                </h2>
                {activeItem.description && (
                  <p className="text-violet-400 font-Kalamayka text-base md:text-lg font-medium">
                    {activeItem.description}
                  </p>
                )}
              </motion.div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-violet-950/60 p-1 ring-1 ring-violet-500/30 backdrop-blur-md">
              <button
                onClick={handlePrev}
                className="rounded-full p-3 text-violet-300 transition hover:bg-violet-500/20 hover:text-white active:scale-95"
                aria-label="Previous"
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="min-w-[40px] text-center text-xs font-mono text-violet-400">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
